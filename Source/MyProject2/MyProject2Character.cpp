...
void AMyProject2Character::Tick(float DeltaTime)
{
	...
	float dotProduct = FVector::DotProduct(playerToSensor, playerForward);

	if (dotProduct >= threshold)
	{
		lineColor = FColor::Purple;
		GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Emerald, TEXT("Hello"));
	}
	else
	{
...
FVector AMyProject2Character::GetLocation()
{
	//return GetActorLocation();
	return FP_Gun->GetComponentLocation();
}

FVector AMyProject2Character::GetForward()
{
	//return GetActorForwardVector();
	return FP_Gun->GetRightVector();
}

FVector AMyProject2Character::GetSensorLocation()
{
	return sensor->GetActorLocation();
}

FVector AMyProject2Character::GetSensorForward()
{
